/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/docs/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { SzenarioWhereUniqueInput } from "../../szenario/base/SzenarioWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";
@InputType()
class SzenarioUpdateManyWithoutMeasuresInput {
  @Field(() => [SzenarioWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [SzenarioWhereUniqueInput],
  })
  connect?: Array<SzenarioWhereUniqueInput>;

  @Field(() => [SzenarioWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [SzenarioWhereUniqueInput],
  })
  disconnect?: Array<SzenarioWhereUniqueInput>;

  @Field(() => [SzenarioWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [SzenarioWhereUniqueInput],
  })
  set?: Array<SzenarioWhereUniqueInput>;
}
export { SzenarioUpdateManyWithoutMeasuresInput };
